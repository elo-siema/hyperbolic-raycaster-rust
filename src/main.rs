...
use sdl2::{pixels::Color, render::Texture, render::TextureCreator, render::WindowCanvas, video::WindowContext};
...
let mut texture = match texture_creator
    ...
    #[cfg(target_os = "emscripten")]
    {
        ...
        emscripten::emscripten::set_main_loop_callback(move || {
            
            main_loop(&mut window, &mut renderer,  &mut initial_run);
        });
    }
    //box_forget(window);
    //box_forget(renderer);
...
fn main_loop(window: &mut Window, renderer: &mut Renderer, initial_run: &mut bool) {
    ...
    let mut texture = match texture_creator
        ...
        texture.with_lock(None, |buffer: &mut [u8], pitch: usize| {
            renderer.render(&mut Canvas::new(buffer, pitch, width as usize, height as usize));
        }).unwrap();
